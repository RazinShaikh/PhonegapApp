...
function addArticlesLatest(data) {
    for (var i = 0; i < data.length; i++) {
        ...
        htmlStr =   '<div class="card myCard" onclick="viewArticle(' + data[i].id + ')">'+
                    '	<div class="card-content">'+
                    '		<div class="card-content-inner">'+
                    '			<div class="row">'+
                    '				<div class="col-50">'+
                    '                   <img height="90" id="img_latest_' + i + '" src="" width="147">'+
                    '               </div>'+
                    '				<div class="col-50">' + data[i].title + '</div>'+
                    '			</div>'+
                    '		</div>'+
                    '	</div>'+
                    '</div>';
...
function addArticlesHot(data) {
    for (var i = 0; i < data.length; i++) {
        htmlStr =   '<div class="card myCard" onclick="viewArticle(' + data[i].id + ')">'+
                    ...
                    '			<div class="row">'+
                    '				<div class="col-50">'+
                    '                   <img height="90" id="img_hot_' + i + '" src="" width="147">'+
                    '               </div>'+
                    '				<div class="col-50">' + data[i].title + '</div>'+
                    '			</div>'+
                    '		</div>'+
...
function loadArticleList() {
    ...
    $$.ajax({
        ...
        type: 'GET',
        cache: true,
        url: 'http://192.168.0.111:8080/activity.json',
    ...
    $$.ajax({
        ...
        type: 'GET',
        cache: true,
        url: 'http://192.168.0.111:8080/thumbs.json',
        success: function(data) {
...
function viewArticle(id) {
    ...
    $$.ajax({
        ...
        type: 'GET',
        cache: true,
        url: 'http://192.168.0.111:8080/activity/'+id+'.json',
        success: function(data) {
